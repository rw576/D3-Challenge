...
d3.csv('health_data.csv',function(err, health_data) {
    ...
    xLinearScale.domain([7,d3.max(health_data, function(data) {
            return +data.poverty;
        }),
    ]);
    yLinearScale.domain([0, d3.max(health_data, function(data) {
        return +data.smokes;
        }),
    ]);
